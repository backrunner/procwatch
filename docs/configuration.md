...
`log.max_size_bytes` enables log rotation for stdout and stderr; `log.retain` controls how many rotated files are kept. Runtime rotation is active when Promon remains attached as supervisor, such as `daemon`, `watch`, or `start --wait`. Direct detached `start` still performs startup-time rotation. `log.merge: true` writes stderr to the stdout log path.
