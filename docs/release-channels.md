...
The published npm package must also include the Node-side runtime assets Promon needs at execution time:
...
`packages/promon/scripts/sync-assets.mjs` populates these assets before `npm pack` and `npm publish`.
